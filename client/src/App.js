...
class App extends Component {
  ...
  getWeather = () => { //takes current latitude and longitude as parameters to be used in API call
    fetch(
  ...
  getStyle = () => {
    ...
    if (this.state.weather.includes("clear") && this.state.time === "day") {
      ...
      image = `url('${stormy}')`
    } else if (this.state.weather.includes("rain") || this.state.weather.includes("drizzle")) {
      image = `url('${rainy}')`
    } 
    return {
  ...
  getLocation = () => {
    return new Promise((resolve) => {
      navigator.geolocation.getCurrentPosition(function (position) {
        ...
        if (location.lon !== "") { //resolves if the location object is not empty, this ensures the program waits for the location call to complete
          resolve();
        }
      });
    });
